...
func (nc *DefaultNodeNetworkController) Start(ctx context.Context) error {
	...
	if config.OvnKubeNode.Mode != types.NodeModeDPUHost && config.OVNKubernetesFeature.EnableInterconnect && nc.sbZone != types.OvnDefaultZone && !util.HasNodeMigratedZone(node) && types.TransportFromINI(config.Default.Transport) != types.NetworkTransportNoOverlay {
...
func (nc *DefaultNodeNetworkController) validateVTEPInterfaceMTU() error {
	...
	for _, ip := range ovnEncapIps {
		...
		// calc required MTU
		var requiredMTU int
		if config.Gateway.SingleNode || types.TransportFromINI(config.Default.Transport) == types.NetworkTransportNoOverlay {
